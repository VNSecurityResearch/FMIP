...
bool MIPF::OnInit() // Entry point of the program.
{
#ifdef _NDEBUG
	...
	MIPF::SetPrivilege(hToken, L"SeDebugPrivilege", TRUE);
	::CloseHandle(hToken);
...
BOOL MIPF::IsProcessWoW64(const HANDLE hProcess)
{
	BOOL bIsWow64 = FALSE;
...
*/
LPCVOID MIPF::FindPrivateERWRegion(HANDLE hProcess, LPCVOID ptrRegionBase)
{
...
void MIPF::MakeTreeItemsInRemoteInstance(HWND hwndDestWindow, HANDLE hProcess, const PROCESS_NAME_AND_PID&  ProcessNamePId)
{
	MakeTreeItemsAboutAProcess(hwndDestWindow, nullptr, nullptr, hProcess, ProcessNamePId);
}

BOOL MIPF::FillTreeCtrl(MIPF_TreeCtrl* ptrTreeCtrl)
{
	...
	do
	{
		...
		if (MIPF::IsProcessWoW64(hProcess) == TRUE) // the process to be examined (with hProcess) is a X86 one running under WoW64, so...
		{
